...
LOG_FILE = 'prediction_log_rf.txt' # Random Forest için ayrı log dosyası

# Modellerin ve etiket haritalarının ana dizini
# Dizin, daha önce eğittiğimiz Random Forest model dosyalarını (pkl) içermelidir.
...
language_configs = {
    # Random Forest EN ve AR tek el (42 özellik) kullanacak
    'EN': {'model_file': 'random_forest_EN.pkl', 'label_map_file': 'label_map_EN.pkl', 'feature_size': 42},
    'AR': {'model_file': 'random_forest_AR.pkl', 'label_map_file': 'label_map_AR.pkl', 'feature_size': 42},
    # Random Forest TR tek veya iki el (84 özellik) kullanacak
    'TR': {'model_file': 'random_forest_TR.pkl', 'label_map_file': 'label_map_TR.pkl', 'feature_size': 84}, 
}
...
def load_resources(lang):
    """Belirtilen dile ait Random Forest modelini ve etiket haritasını yükler."""
    ...
    if not os.path.exists(model_path) or not os.path.exists(label_map_path):
        print(f"Hata: {lang} diline ait Random Forest modeli veya etiket dosyası bulunamadı.")
    ...
    try:
        ...
        print(f"[{lang}] dilindeki Random Forest modeli ve etiketler başarıyla yüklendi. Beklenen boyut: {config['feature_size']}")
        return True
...
# UI ayarları (Tkinter)
root = tk.Tk()
root.title("Hand Gesture Recognition - Random Forest")
    ...
    if results.multi_hand_landmarks:
        ...
        if len(final_data_vector) == expected_size:
            ...
            try:
                ...
                # Not: Random Forest için kısaltma olarak 'RF' kullanıldı.
                time_text = f"RF: {duration_ms:.2f} ms"
...
# Butonlar 
# Random Forest modelini kullandığımızı belirtmek için buton isimleri güncellendi.
...
en_button = tk.Button(root, text="EN (RF - 1H)", command=lambda: change_language_and_load('EN'))
en_button.grid(row=3, column=1, padx=10, pady=10)
ar_button = tk.Button(root, text="AR (RF - 1H)", command=lambda: change_language_and_load('AR'))
ar_button.grid(row=3, column=0, padx=10, pady=10)
tr_button = tk.Button(root, text="TR (RF - 1/2H)", command=lambda: change_language_and_load('TR'))
...
if __name__ == '__main__':
    ...
    if load_resources(current_language):
        ...
        print("Başlatma başarısız oldu. Lütfen Random Forest model dosyalarının doğru yolda ve doğru formatta olduğundan emin olun.")
