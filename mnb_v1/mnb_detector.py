...
# Tahmin sürelerinin kaydedileceği dosya (MNB'ye göre güncellendi)
LOG_FILE = 'prediction_log_mnb.txt' 

# Modellerin ve etiket haritalarının ana dizini (MNB'ye göre güncellendi)
# Bu dizin, daha önce eğittiğimiz MNB model dosyalarını (pkl) içermelidir.
MODELS_DIR = r'C:\Users\ridva\.conda\envs\vmamba_env\vmamba_proje\mnb_v1' 

# Dil modelleri, dosya adları ve beklenen özellik vektörü boyutları
language_configs = {
    # MNB modelleri için dosya adları güncellendi
    'EN': {'model_file': 'mnb_EN.pkl', 'label_map_file': 'label_map_EN.pkl', 'feature_size': 42},
    'AR': {'model_file': 'mnb_AR.pkl', 'label_map_file': 'label_map_AR.pkl', 'feature_size': 42},
    'TR': {'model_file': 'mnb_TR.pkl', 'label_map_file': 'label_map_TR.pkl', 'feature_size': 84}, 
}
...
def load_resources(lang):
    """Belirtilen dile ait MNB modelini ve etiket haritasını yükler."""
    ...
    if not os.path.exists(model_path) or not os.path.exists(label_map_path):
        print(f"Hata: {lang} diline ait MNB modeli veya etiket dosyası bulunamadı.")
    ...
    try:
        ...
        print(f"[{lang}] dilindeki MNB modeli ve etiketler başarıyla yüklendi. Beklenen boyut: {config['feature_size']}")
        return True
...
# UI ayarları (Tkinter)
# Başlık MNB'ye göre güncellendi
root = tk.Tk()
root.title("Hand Gesture Recognition - Multinomial Naive Bayes (MNB)")
    ...
    if results.multi_hand_landmarks:
        ...
        if len(final_data_vector) == expected_size:
            ...
            try:
                ...
                # İşlem süresini çerçeveye yaz (MNB'ye göre güncellendi)
                time_text = f"MNB: {duration_ms:.2f} ms"
        ...
        # Yeni bir el algılandığında veya karakter değiştirildiğinde hand_start_time sıfırlanır.
        if hand_start_time is None:
            ...
            current_character = "" # Karakter eklendikten sonra tahmin karakterini temizle
            hand_start_time = None
...
# Butonlar 
# MNB modelini kullandığımızı belirtmek için buton isimleri güncellendi.
...
# Buton metinleri MNB'ye göre güncellendi
en_button = tk.Button(root, text="EN (MNB - 1H)", command=lambda: change_language_and_load('EN'))
en_button.grid(row=3, column=1, padx=10, pady=10)
ar_button = tk.Button(root, text="AR (MNB - 1H)", command=lambda: change_language_and_load('AR'))
ar_button.grid(row=3, column=0, padx=10, pady=10)
tr_button = tk.Button(root, text="TR (MNB - 1/2H)", command=lambda: change_language_and_load('TR'))
...
if __name__ == '__main__':
    ...
    if load_resources(current_language):
        ...
        print("Başlatma başarısız oldu. Lütfen MNB model dosyalarının doğru yolda ve doğru formatta olduğundan emin olun.")
