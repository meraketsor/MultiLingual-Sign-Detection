...
from sklearn.naive_bayes import MultinomialNB # Multinomial Naive Bayes için gerekli kütüphane
...
# KULLANICI NOTU: Çıktılar artık "mnb_v1" klasörüne kaydedilecektir.
MODEL_DIR = Path(r'C:\Users\ridva\.conda\envs\vmamba_env\vmamba_proje\mnb_v1')
MODEL_DIR.mkdir(parents=True, exist_ok=True) # Klasör yoksa oluştur

# Değerlendirme raporunun kaydedileceği dosya yolu
EVAL_FILE = MODEL_DIR / 'mnb_evaluation.txt' # Dosya adı MNB'yi yansıtacak şekilde güncellendi
...
# MNB Hiperparametreleri
MNB_PARAMS = {
    'alpha': 1.0, # Laplace düzeltme (smoothing) parametresi. Pozitif olmalı.
}
...
# --- ÖZELLİK ÇIKARMA (SVM/LR script'leri ile aynı) ---
...
# --- EĞİTİM VE DEĞERLENDİRME (MNB) ---

def train_and_evaluate_mnb(language_code, data, labels):
    """
    Multinomial Naive Bayes (MNB) modelini eğitir, değerlendirir ve sonuçları kaydeder.
    """
    if data is None or labels is None or len(data) == 0:
        print(f"[{language_code}] Eğitim için yeterli veri bulunamadı. Atlanıyor.")
        return None
        
    print(f"[{language_code}] Multinomial Naive Bayes (MNB) modeli eğitiliyor...") 
    ...
    # 2. MNB için POZİTİF ÖLÇEKLENDİRME (Negatif hata önlemi)
    # Multinomial Naive Bayes, özelliklerin negatif olmasını kabul etmez.
    # Veriyi pozitif alana taşımak için en küçük değeri çıkarıyoruz: x_yeni = x - min(x)
    min_val = np.min(data_prepared)
    
    # Sadece negatif değerler varsa dönüştürme yap. Genellikle bu özellik vektörleri negatif içerir.
    if min_val < 0:
        data_scaled = data_prepared - min_val # Tüm veriyi pozitif alana kaydır
        print(f"[{language_code}] Negatif değerler önlendi. Veri pozitif alana kaydırıldı (En küçük değer: {min_val:.4f}).")
    else:
        data_scaled = data_prepared
        print(f"[{language_code}] Negatif değerler bulunmadı. Pozitif alana kaydırma yapılmadı.")

    # 3. Etiketleri sayısal değerlere dönüştür
    le = LabelEncoder()
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        data_scaled, # Ölçeklenmiş (pozitif) veri kullanılıyor
        numeric_labels, 
        test_size=0.2, 
        shuffle=True, 
    ...
    # 5. MNB modelini oluştur ve eğit
    # MultinomialNB, sayım verileri için tasarlanmıştır, ancak pozitif ölçeklenmiş 
    # sürekli veriler (bizim durumumuzdaki gibi) üzerinde iyi bir baseline sınıflandırıcı olarak kullanılabilir.
    model = MultinomialNB(**MNB_PARAMS) 
    ...
    # 7. Doğruluk ve Sınıflandırma Raporu hesapla
    ...
    # 8. Sonuçları dosyaya yaz
    evaluation_content = f"--- {language_code} İşaret Dili Modeli Değerlendirme Raporu (Multinomial Naive Bayes - MNB) ---\n"
    evaluation_content += f"Model Dosyası: mnb_{language_code}.pkl\n" 
    evaluation_content += f"Özellik Boyutu: {data_prepared.shape[1]}\n"
    evaluation_content += f"Pozitif Ölçeklendirme Uygulandı: Evet (MNB Gereksinimi)\n" # Önlem belirtildi
    evaluation_content += f"MNB Alfa (Smoothing): {MNB_PARAMS['alpha']}\n"
    ...
    # 9. Modeli .pkl dosyası olarak kaydet
    model_filename = MODEL_DIR / f'mnb_{language_code}.pkl' 
...
if __name__ == '__main__':
    ...
    for lang in LANGUAGES:
        ...
        duration = train_and_evaluate_mnb(lang, data, labels)
    ...
    total_summary_content = f"\n\n--- GENEL EĞİTİM ÖZETİ (MULTINOMIAL NAIVE BAYES - MNB) ---\n"
    ...
    # Sonuç
    print("\n\n--- TÜM İŞARET DİLİ MODELLERİNİN (MNB) EĞİTİMİ TAMAMLANDI ---")
