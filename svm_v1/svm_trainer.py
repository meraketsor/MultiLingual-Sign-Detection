...
from sklearn.svm import SVC # Support Vector Machine (SVC) için gerekli kütüphane
...
# KULLANICI NOTU: Çıktılar artık "svm_v1" klasörüne kaydedilecektir.
MODEL_DIR = Path(r'C:\Users\ridva\.conda\envs\vmamba_env\vmamba_proje\svm_v1') # DİZİN GÜNCELLENDİ
MODEL_DIR.mkdir(parents=True, exist_ok=True) # Klasör yoksa oluştur

# Değerlendirme raporunun kaydedileceği dosya yolu
EVAL_FILE = MODEL_DIR / 'svm_evaluation.txt'
...
# SVM (SVC) hiperparametreleri
# SVC çok sınıflı sınıflandırmayı otomatik olarak destekler (One-vs-One veya One-vs-Rest stratejisiyle).
SVM_PARAMS = {
    'kernel': 'linear',       # Lineer çekirdek hızlıdır ve özellikler göreceli konumu temsil ettiği için iyi çalışır.
    'C': 0.1,                 # Düzenleme parametresi (C'nin küçük olması daha yumuşak sınırlar oluşturur)
    'random_state': 42,       # Tekrarlanabilir sonuçlar için
    'probability': False,     # Tahmin olasılığına gerek yoksa 'False' hız artırır.
}
...
# --- EĞİTİM VE DEĞERLENDİRME (SVM) ---

def train_and_evaluate_svm(language_code, data, labels):
    """
    Support Vector Machine (SVC) modelini eğitir, değerlendirir ve sonuçları kaydeder.
    """
    if data is None or labels is None or len(data) == 0:
        print(f"[{language_code}] Eğitim için yeterli veri bulunamadı. Atlanıyor.")
        return None
        
    print(f"[{language_code}] Support Vector Machine (SVC) modeli eğitiliyor...") 
    ...
    # 4. SVM (SVC) modelini oluştur ve eğit
    ...
    evaluation_content = f"--- {language_code} İşaret Dili Modeli Değerlendirme Raporu (Support Vector Machine - SVC) ---\n"
    evaluation_content += f"Model Dosyası: svm_{language_code}.pkl\n" 
    evaluation_content += f"Özellik Boyutu: {data_prepared.shape[1]}\n"
    evaluation_content += f"SVM Kernel Tipi: {SVM_PARAMS['kernel']}\n"
    evaluation_content += f"SVM Düzenleme Parametresi (C): {SVM_PARAMS['C']}\n"
    ...
    model_filename = MODEL_DIR / f'svm_{language_code}.pkl' 
...
if __name__ == '__main__':
    ...
    for lang in LANGUAGES:
        ...
        duration = train_and_evaluate_svm(lang, data, labels)
    ...
    total_summary_content = f"\n\n--- GENEL EĞİTİM ÖZETİ (SUPPORT VECTOR MACHINE - SVC) ---\n"
    ...
    # Sonuç
    print("\n\n--- TÜM İŞARET DİLİ MODELLERİNİN (SVM) EĞİTİMİ TAMAMLANDI ---")
