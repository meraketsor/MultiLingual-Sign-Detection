...
ARABIC_CHAR_MAP = {
    ...
    'kaaf': 'ك',
    'khaa': 'خ',
    'la': 'لا', # Genellikle aynı harfi temsil edebilir, modelinize göre düzeltin.
    'laam': 'ل',
    'meem': 'م',
    'nun': 'ن',
    'ra': 'ر',
    ...
    'toot': 'ۃ', # Alternatif Tı harfi için, modelinize göre düzeltin.
    'waw': 'و',
    'ya': 'ي',
    'yaa': 'ي',
    'zay': 'ز',
...
# Tahmin sürelerinin kaydedileceği dosya (SVM'ye göre güncellendi)
LOG_FILE = 'prediction_log_svm.txt' 

# Modellerin ve etiket haritalarının ana dizini (SVM'ye göre güncellendi)
MODELS_DIR = r'C:\Users\ridva\.conda\envs\vmamba_env\vmamba_proje\svm_v1' 

# Dil modelleri, dosya adları ve beklenen özellik vektörü boyutları
language_configs = {
    # SVM modelleri için dosya adları güncellendi
    'EN': {'model_file': 'svm_EN.pkl', 'label_map_file': 'label_map_EN.pkl', 'feature_size': 42},
    'AR': {'model_file': 'svm_AR.pkl', 'label_map_file': 'label_map_AR.pkl', 'feature_size': 42},
    'TR': {'model_file': 'svm_TR.pkl', 'label_map_file': 'label_map_TR.pkl', 'feature_size': 84}, 
}
...
def load_resources(lang):
    """Belirtilen dile ait SVM modelini ve etiket haritasını yükler."""
    ...
    if not os.path.exists(model_path) or not os.path.exists(label_map_path):
        print(f"Hata: {lang} diline ait SVM modeli veya etiket dosyası bulunamadı.")
    ...
    try:
        ...
        print(f"[{lang}] dilindeki SVM modeli ve etiketler başarıyla yüklendi. Beklenen boyut: {config['feature_size']}")
        return True
...
# UI ayarları (Tkinter)
# Başlık SVM'ye göre güncellendi
root = tk.Tk()
root.title("Hand Gesture Recognition - Support Vector Machine (SVM)")

char_var = tk.StringVar()
sentence_var = tk.StringVar()

char_label = tk.Label(root, text="Tahmin Edilen Karakter:", font=('Arial', 16))
char_entry = tk.Entry(root, textvariable=char_var, font=ui_font, state='readonly', width=5, justify='right') 
sentence_label = tk.Label(root, text="Oluşan Kelime:", font=('Arial', 16))
sentence_entry = tk.Entry(root, textvariable=sentence_var, font=ui_font, state='readonly', width=20, justify='right') 
...
def get_display_character(latin_label, lang):
    """
    Modelin verdiği Latince etiketi dil bazında doğru karaktere çevirir.
    """
    if lang == 'AR':
    ...
    if results.multi_hand_landmarks:
        ...
        if len(final_data_vector) == expected_size:
            ...
            try:
                ...
                # Modelden gelen Latince etiketi al
                predicted_latin_label = labels_dict.get(int(prediction[0]), 'UNKNOWN')
                
                # Ekranda gösterilecek karakteri al
                ...
                # Tahmin süresini metin dosyasına logla
                ...
                # İşlem süresini çerçeveye yaz (SVM'ye göre güncellendi)
                time_text = f"SVM: {duration_ms:.2f} ms"
        ...
        if hand_start_time is None:
            ...
            if current_character not in ["", "?", "UNKNOWN"]:
                ...
                if current_language == 'AR':
                    ...
                    # Diğer dillerde (TR, EN) karaketere sona eklenir
                    new_sentence = sentence_var.get() + current_character
...
# Butonlar 
# Buton metinleri SVM'ye göre güncellendi
...
en_button = tk.Button(root, text="EN (SVM - 1H)", command=lambda: change_language_and_load('EN'))
en_button.grid(row=3, column=1, padx=10, pady=10)
ar_button = tk.Button(root, text="AR (SVM - 1H)", command=lambda: change_language_and_load('AR'))
ar_button.grid(row=3, column=0, padx=10, pady=10)
tr_button = tk.Button(root, text="TR (SVM - 1/2H)", command=lambda: change_language_and_load('TR'))
...
if __name__ == '__main__':
    ...
    if load_resources(current_language):
        ...
        print("Başlatma başarısız oldu. Lütfen SVM model dosyalarının doğru yolda ve doğru formatta olduğundan emin olun.")
