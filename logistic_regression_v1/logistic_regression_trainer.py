...
# KULLANICI NOTU: Çıktılar artık "logistic_regression_v1" klasörüne kaydedilecektir.
MODEL_DIR = Path(r'C:\Users\ridva\.conda\envs\vmamba_env\vmamba_proje\logistic_regression_v1')  # DİZİN GÜNCELLENDİ
MODEL_DIR.mkdir(parents=True, exist_ok=True)  # Klasör yoksa oluştur

# Değerlendirme raporunun kaydedileceği dosya yolu
EVAL_FILE = MODEL_DIR / 'logistic_regression_evaluation.txt'
...
# Logistic Regression hiperparametreleri
LOGISTIC_PARAMS = {
    'C': 1.0,                    # Ters düzenleme parametresi (küçük değerler daha güçlü düzenleme)
    'solver': 'lbfgs',           # Optimizasyon algoritması (lbfgs küçük/orta veri setleri için iyi)
    'max_iter': 1000,            # Maksimum iterasyon sayısı
    'multi_class': 'multinomial', # Çok sınıflı sınıflandırma için
    'random_state': 42,          # Tekrarlanabilir sonuçlar için
    'verbose': 0                 # Günlük seviyesi (0: sessiz)
}
...
def train_and_evaluate_logistic_regression(language_code, data, labels):
    ...
    print(f"[{language_code}] Logistic Regression modeli eğitiliyor...") 
    ...
    evaluation_content += f"Logistic Regression C Parametresi: {LOGISTIC_PARAMS['C']}\n"
    evaluation_content += f"Logistic Regression Solver: {LOGISTIC_PARAMS['solver']}\n"
    ...
    model_filename = MODEL_DIR / f'logistic_regression_{language_code}.pkl' 
...
if __name__ == '__main__':
    ...
    for lang in LANGUAGES:
        ...
        duration = train_and_evaluate_logistic_regression(lang, data, labels)
    ...
    # Sonuç
    print("\n\n--- TÜM İŞARET DİLİ MODELLERİNİN (LOGISTIC REGRESSION) EĞİTİMİ TAMAMLANDI ---")