...
from sklearn.linear_model import LogisticRegression # Logistic Regression için gerekli kütüphane
...
# KULLANICI NOTU: Burası Logistic Regression için istediğiniz yoldur.
MODEL_DIR = Path(r'C:\Users\ridva\.conda\envs\vmamba_env\vmamba_proje\logistic_regression_v1')
MODEL_DIR.mkdir(parents=True, exist_ok=True) # Klasör yoksa oluştur

# Değerlendirme raporunun kaydedileceği dosya yolu
EVAL_FILE = MODEL_DIR / 'logistic_regression_evaluation.txt' # Dosya adı güncellendi
...
# Logistic Regression hiperparametreleri (Basit ve hızlı ayarlar)
LR_PARAMS = {
    'solver': 'liblinear', # Küçük veri setleri için iyi bir seçim
    'max_iter': 500,       # Yeterli iterasyon sayısı
...
# --- ÖZELLİK ÇIKARMA (Random Forest script'inizdeki ile aynı) ---
...
# --- EĞİTİM VE DEĞERLENDİRME (Logistic Regression) ---

def train_and_evaluate_logistic_regression(language_code, data, labels):
    """
    ...
    print(f"[{language_code}] Logistic Regression modeli eğitiliyor...") # Model adı güncellendi
    ...
    # 4. Logistic Regression modelini oluştur ve eğit
    model = LogisticRegression(**LR_PARAMS) # Sınıflandırıcı değiştirildi
    ...
    evaluation_content = f"--- {language_code} İşaret Dili Modeli Değerlendirme Raporu (Logistic Regression) ---\n" # Başlık güncellendi
    evaluation_content += f"Model Dosyası: logistic_regression_{language_code}.pkl\n" # Dosya adı güncellendi
    evaluation_content += f"Özellik Boyutu: {data_prepared.shape[1]}\n" # Kullanılan boyutu rapora ekle
    evaluation_content += f"Logistic Regression Max Iterasyon: {LR_PARAMS['max_iter']}\n" # Parametre güncellendi
    ...
    model_filename = MODEL_DIR / f'logistic_regression_{language_code}.pkl' # Dosya adı güncellendi
...
if __name__ == '__main__':
    ...
    for lang in LANGUAGES:
        ...
        duration = train_and_evaluate_logistic_regression(lang, data, labels) # Fonksiyon adı güncellendi
    ...
    total_summary_content = f"\n\n--- GENEL EĞİTİM ÖZETİ (LOGISTIC REGRESSION) ---\n" # Başlık güncellendi
    ...
    # Sonuç
    print("\n\n--- TÜM İŞARET DİLİ MODELLERİNİN (LOGISTIC REGRESSION) EĞİTİMİ TAMAMLANDI ---") # Başlık güncellendi
